...
Jc = J * curlb_Bx
Jc_core = np.concatenate((Jc[:, (j11+1):(j21+1)], Jc[:, (j12+1):(j22+1)]), axis=1)
...
bxcvx2 = (np.clip(bxcvx, 0.0, None) * factor_edge +   # Positive parts
          np.clip(bxcvx, None, 0.0) / factor_edge)    # Negative parts

# Go through core flux surfaces
for i in range(ixcore):
    # Inner core
    bxcvx2[i, (j11+1):(j21+1)] = (np.clip(bxcvx[i, (j11+1):(j21+1)], 0.0, None) * factor[i] +
                                  np.clip(bxcvx[i, (j11+1):(j21+1)], None, 0.0) / factor[i])
    
    # Outer core
    bxcvx2[i, (j12+1):(j22+1)] = (np.clip(bxcvx[i, (j12+1):(j22+1)], 0.0, None) * factor[i] +
...
Jc2 = J * 2 * bxcvx2 / B
Jc2_core = np.concatenate((Jc2[:, (j11+1):(j21+1)], Jc2[:, (j12+1):(j22+1)]), axis=1)
